...
{
  const std::size_t M = N; //for simplicity in tasks 
  const std::size_t K = N; //for simplicity in tasks 
//std::cout << M  << std::endl; 
...
//std::cout << "finish malloc " << std::endl;
//float *v4;  
//cudaMalloc(&v4, N*M * sizeof(float));
  ...
  const unsigned int n_blocks = (N*N + block_size  ) / block_size;
...
//  set_vector<<<grid_dim, block_dim>>>(M*K, 0.f, v3);

// std::cout<<"finish set" << std::endl;
 set_vector<<<n_blocks, block_dim>>>(M*N, 1.f, v1);
  set_vector<<<n_blocks, block_dim>>>(N*K, 1.f, v2);
  set_vector<<<n_blocks, block_dim>>>(M*K, 0.f, v3);
...
//transpose<<<grid_dim, block_dim>>>(M, N, v1, v4);
     //compute_matrix_matrix_product<<<grid_dim, block_dim>>>(N, v1, v2, v3);
compute_matrix_matrix_product<<<n_blocks, block_size>>>(N, v1, v2, v3);

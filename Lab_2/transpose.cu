                     ...
                     const long long   repeat)
{
  ...
  cudaMalloc(&v2, M * sizeof(float));
  //cudaMalloc(&v3, N*K * sizeof(float));


  const unsigned int n_blocks = (N + block_size - 1) / block_size;

  set_vector<<<n_blocks, block_size>>>(M*N, 17.f, v1);
  set_vector<<<n_blocks, block_size>>>(N*M, 0.f, v2);
  ...
  for (unsigned int t = 0; t < n_tests; ++t)
    { 
      ...
      transposeSimple<<<n_blocks, block_size>>>(N, M, v1, v2);
  ...
  cudaMemcpy(result_host.data(), v2, M*K * sizeof(float), cudaMemcpyDeviceToHost);
  
//std::cout<<result_host[0]<<std::endl;
/**if ((result_host[0] + result_host[N - 1]) != 17.f*42.f*N)
...
//  cudaFree(v3);
  ...
  std::cout << "STREAM triad of size " << std::setw(8) << N
            ...
            << 1e-9 * 3 * sizeof(float) * M * N / best << " GB/s" << std::endl;
}

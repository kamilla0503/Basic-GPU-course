...
Kokkos::TeamPolicy<> policy(N, Kokkos::AUTO, Kokkos::AUTO);
//Kokkos::TeamPolicy policy(league_size=N, team_size=AUTO, vector_size=AUTO);
        ...
        // Calculate bandwidth.
        double Gbytes = 1.0e-9 * double(sizeof(double) * (25 * N));

        std::cout << "size " << std::setw(8) << N << " "
                  << 1e-6 * 25 * N / (time / nrepeat)

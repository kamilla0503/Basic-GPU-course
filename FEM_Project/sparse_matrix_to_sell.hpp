                             ...
                             Number *y)
{
  ...
  if (idx < N)
  {
   Number sum = 0;
   //weak 
        for (std::size_t i = row_starts[idx]; i < row_starts[idx+1]; ++i)
         {
         if (values[i]!=0) sum += values[i] * x[column_indices[i]];
         }
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        const unsigned int n_blocks =
          (n_entries + block_size - 1) / block_size;
          compute_spmv<<<n_blocks, block_size>>>(n_entries,row_starts, column_indices,

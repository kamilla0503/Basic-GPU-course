                             ...
                             Number *y)
{
  // TODO implement for GPU
    //int idx = threadIdx.x; 
    int row =threadIdx.x + blockIdx.x * blockDim.x;  // outer
    //int lrow = threadIdx.x % C; // inner;
    
    if( row < N/C ) {
        int start = row_starts[row/C];
        int border = (row_starts[row/C+1]-start)/C;
        //Number sum = 0;
        for (int j = 0; j < border; j++) { 
        
            for (int k=0; k<C; k++)
            {
            int col = column_indices[start+C *j + k ];
            y[start+C*j + k]+= values[start+C*j + k ];     
            }
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        const unsigned int n_blocks =
          (n_entries + block_size - 1) / block_size/C;
          compute_spmv<<<n_blocks, block_size>>>(n_entries,row_starts, column_indices, col_widths,

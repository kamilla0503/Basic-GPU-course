...
#include "sparse_matrix_to_sell.hpp"
#include "vector.hpp"
...
//#pragma omp parallel for collapse(2)
  ...
  //SparseMatrix<Number> sparse(row_lengths, memory_space, MPI_COMM_SELF);
    ...
    //std::vector<unsigned int>      number_in_rows;
//#pragma omp parallel
  //{
   //col_widths = col_widths_;
  std::size_t  *number_in_rows=new std::size_t[size*size*size];
  //col_widths__ =col_widths_; 
  /*for (int i=0;i<col_widths_.size();i++)
  {
  	col_widths[i]=col_widths_[i];
  }*/
//#pragma omp for collapse(2)
                  ...
                  if (iz + iiz >= 0 && iz + iiz < size && iy + iiy >= 0 &&
                      iy + iiy < size && ix + iix >= 0 && ix + iix < size)
                    {
                      col_indices.push_back(row + (iiz * size + iiy) * size +
                                            iix);
                                            //std::cout << entries[count] * scale << std::endl;
                      values.push_back(entries[count] * scale);
                      n_in_row +=1;                      
                    }
          
            if (n_in_row != row_lengths[row])
              exit(EXIT_FAILURE);
               number_in_rows[row] = n_in_row;
   ...
   int calc_size = 0; 
     std::vector<unsigned int> sizes_(size * size * size);
     for (int i = 0; i<size*size*size/C;i++)
  {
    int max_ = 0;
  	for (int j=0; j<C; j++)
  	{
  	if (number_in_rows[i*C+j] > max_) max_ = number_in_rows[i*C+j];
  	}
  	calc_size += ( max_*C);
  ...
  /*   for (int i = 0; i<size*size*size/C;i++)
  {
  std::cout << sizes_[i ] << " " << number_in_rows[i ] << std::endl;
  }*/
 
 SparseMatrix<Number> sparse(sizes_,  memory_space, MPI_COMM_SELF);  
       std::vector<unsigned int> col_indices_temp;;
    std::vector<Number>       values_temp;
 
 int cur_pos = 0;
  for (int i = 0; i<size*size*size/C;i++)
  {
  	col_indices_temp.clear();
	values_temp.clear();
  int max_ = 0;
  	for (int j=0; j<C; j++)
  	{
  	...
  	for (int j=0; j<C; j++)
  	{
  	  	...
  	  	col_indices_temp.clear();
	values_temp.clear();
  	    for (int k=cur_pos; k < cur_pos+number_in_rows[i*C+j]; k++)
  	    {
  	    //std:: cout << " k " << k << " " <<col_indices[k] << std::endl;
  	     values_temp.push_back(values[k]);
	     col_indices_temp.push_back(col_indices[k]);
  	    }
  	    cur_pos+=number_in_rows[i*C+j];
  	    //std::cout << number_in_rows[i*C+j] << " " << cur_pos ;
  	 for (int k=number_in_rows[i*C+j]; k < max_  ; k++)
  	    {
  		  	  	values_temp.push_back(0);
	  		col_indices_temp.push_back(0); //0*
  	    }    
	  	/*if (number_in_rows[i*C+j]<= max_){
	  		values_temp.push_back(values[i*C+j]);
	  		col_indices_temp.push_back(col_indices[i*C+j]);
	  		}
	  	else
	  	{
	  	  	values_temp.push_back(0);
	  		col_indices_temp.push_back(0); //0*
	  	  	...
	  	  	sparse.add_row(i*C+j, col_indices_temp, values_temp);
  	
  	/*std::cout << "row " << i << " " << max_ << " " << sizes_[i*C+j] <<std::endl; 
  	
 	std::cout << number_in_rows[i*C+j] << " " <<
	  	col_indices_temp.size() << " " << values_temp.size()
	  	<<std::endl; 
	  	*/
  	}
          ...
          for (unsigned int ix = 0; ix < N; ++ix)
            {
              double local_error =
                PI * PI * 3 *
                  ...
                  std::sin(PI * static_cast<double>(iz + 1) / (N + 1)) -
                dst(iz * N * N + iy * N + ix);
               /* std::cout <<                std::sin(PI * static_cast<double>(ix + 1) / (N + 1)) *
                  std::sin(PI * static_cast<double>(iy + 1) / (N + 1)) *
                  std::sin(PI * static_cast<double>(iz + 1) / (N + 1))  << " " << dst(iz * N * N + iy * N + ix) <<std::endl;*/
              my_error +=

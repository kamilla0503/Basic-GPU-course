...
__global__ void compute_spmv(const int N,
                             const std::size_t *row_starts,
                             const unsigned int *column_indices, const  std::size_t  *col_widths, 
                             const Number *values,
                             const Number *x,
                             Number *y)
{
  // TODO implement for GPU
    //int idx = threadIdx.x; 
    int row = blockIdx.x * blockDim.x;  // outer
    //int lrow = threadIdx.x%C; // inner;
    ...
    if( idx<N ) {
	int e = idx/C;
        int start = row_starts[e];
	int row1 =  e + 1;
	int q = row_starts[row1];
        int border =(q-start);

        for (int j = 0; j < border; j++) { 
        int ci = start+C*j+lrow;
        int col = column_indices[ci];
	//if (values[ci]!=0.)            
	y[idx ]+= values[ci]*x[col];             
...
public:
  ...
  SparseMatrix(const std::vector<unsigned int> &row_lengths,  std::size_t  *col_widths_,
...
//for (int i =0; i<100;i++)
//std::cout << row_starts[i] << std::endl;
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        std::size_t *host_col_widths = col_widths;
        col_widths = 0;
        AssertCuda(cudaMalloc(&col_widths, (n_rows ) * sizeof(std::size_t)));
        AssertCuda(cudaMemcpy(col_widths,
                              host_col_widths,
                              (n_rows ) * sizeof(std::size_t),
                              cudaMemcpyHostToDevice));


        //AssertCuda(cudaMalloc(&col_widths, col_widths_.size()* sizeof(std::size_t)));
#ifndef DISABLE_CUDA
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        AssertCuda(cudaMemcpy(values,
                ...
                AssertCuda(cudaMalloc(&col_widths,
                              n_rows * sizeof( std::size_t )));
        AssertCuda(cudaMemcpy(col_widths,
                              other.col_widths,
                              n_rows * sizeof( std::size_t ),
    ...
    for (unsigned int i = 0; i < columns_of_row.size(); i++)
      {
       ...
       int t = row_starts[row/C] + i*C + row%C;
        //column_indices[row_starts[row] + i] = columns_of_row[i];
       // values[row_starts[row] + i]         = values_in_row[i];
        column_indices[t] = columns_of_row[i];
        values[t]         = values_in_row[i];
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        //the last element is the number of non-zero elements
        const std::size_t n_entries = n_rows;//row_starts[n_rows];
        const unsigned int n_blocks =
          (n_entries + block_size - 1) /block_size;
...
//dim3 grid(int(sqrt())   )
// printf ("%d %d %d \n ", n_rows, m(), n_entries);
          compute_spmv<<<n_blocks, block_size>>>( n_rows,row_starts, column_indices, col_widths,
         values,src.data,dst.data);        
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        exit(EXIT_FAILURE);
        // return dummy
        return SparseMatrix(std::vector<unsigned int>(), col_widths,
                            MemorySpace::CUDA,
                            communicator);
      }
    else
      {
        ...
        SparseMatrix other(row_lengths, col_widths,
                           MemorySpace::CUDA,
                           communicator);
        ...
        AssertCuda(cudaMemcpy(other.values,
                              values,
                              row_lengths[n_rows] * sizeof(Number),
                              cudaMemcpyHostToDevice));
                              
                              AssertCuda(cudaMemcpy(other.col_widths,
                              col_widths,
                              n_rows * sizeof(std::size_t),
                              cudaMemcpyHostToDevice));
        return other;
...
private:
  ...
  MemorySpace   memory_space;
   std::size_t * col_widths;
std::vector<unsigned int> col_widths__;

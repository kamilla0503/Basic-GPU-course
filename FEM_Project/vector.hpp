                              ...
                              Number *destination)
{
  ...
  //vector_update<<<n_blocks, block_size>>>(local_size, my_scalar,other_scalar, other.data, this->data);
   const int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < N)
    destination[idx] = (scalar2*source[idx]+scalar1*destination[idx]); 
}
  ...
  Vector copy_to_device()
  {
    if (memory_space == MemorySpace::CUDA)
    ...
    else
      {
        ...
        cudaMemcpy(other.data, this->data, local_size*sizeof(Number), cudaMemcpyHostToDevice);
        return other;
      }
  }
    ...
    if (memory_space == MemorySpace::CUDA)
      {
    	...
    	cudaMemcpy(other.data, this->data, local_size*sizeof(Number), cudaMemcpyDeviceToHost); 
        return other;
      }
    else
      {
